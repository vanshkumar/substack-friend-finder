...
document.addEventListener('DOMContentLoaded', () => {
  // Check if we're on a Substack page (use callback for Firefox MV2)
...
cancelBtn.addEventListener('click', async () => {
  ...
  // Tell content script to clear its state too (use callback for Firefox MV2)
  chrome.tabs.sendMessage(currentTabId, { type: 'CANCEL_SCAN' }, () => {
    // Ignore errors - content script may not be available
...
async function startScan() {
  ...
  console.log('[SFF Popup] Sending START_SCAN to tab', currentTabId, 'for user', username);

  // Send message to content script (use callback for Firefox MV2 compatibility)
  chrome.tabs.sendMessage(currentTabId, {
    type: 'START_SCAN',
    username
  }, (response) => {
    console.log('[SFF Popup] sendMessage response:', response, 'error:', chrome.runtime.lastError);
...
// Storage functions (wrapped for Firefox MV2 compatibility)
...
function saveScanState(state) {
  return new Promise((resolve) => {
    chrome.storage.local.set({ [SCAN_STATE_KEY]: { ...state, timestamp: Date.now() } }, resolve);
  });
}

function loadScanState() {
  return new Promise((resolve) => {
    chrome.storage.local.get(SCAN_STATE_KEY, (data) => {
      resolve(data[SCAN_STATE_KEY] || null);
    });
  });
}
...
function clearScanState() {
  return new Promise((resolve) => {
    chrome.storage.local.remove(SCAN_STATE_KEY, resolve);
  });
}
